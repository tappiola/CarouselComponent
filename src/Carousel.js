...
export const Carousel = (
    ...
    >
        <div
            onClick={toPrevSlide}
            className={clsx(classes.arrow, classes.arrowInnerLeft, {
                [classes.arrowDisabled]: !infinite && activeIndex === 0,
            })}
        >
        ...
        <div className={classes.carouselItems}
             ...
             style={{
                 ...
                 {[items[items.length - 1], ...items, items[0]].map((item, index) =>
                    <div key={index} className={classes.carouselItem}>{item}</div>
                 )}
        </div>

        <div
            onClick={toNextSlide}
            className={clsx(classes.arrow, classes.arrowInnerRight, {
                [classes.arrowDisabled]: !infinite && activeIndex === items.length - 1,
            })}
        >
            <i className="fa fa-angle-right"/>
        </div>

        <div className={clsx(classes.carouselNav, classes.carouselNavInner)}>
            { items.length > 1 && items.map((_, index) =>
                <span
                    className={clsx(classes.dotIcon, {
                        // Make dot active immediately, including cases with infinite scroll and virtual slides
                        [classes.dotIconActive]: ((activeIndex + items.length) % items.length) === index
                    })}
                    key={index}
